...
expect_equal(unname(obj_1[1]),
             unname(log(obj_2)[1]))

expect_equal(unname(obj_1[2]),
             unname(1/(obj_2)[2]))

## Checks that na.rm works as intended.
expect_equal(coef(mlllogis(small_data)),
             coef(mlllogis(c(small_data, NA), na.rm = TRUE)))

## Is the log-likelihood correct?
est = mlllogis(small_data, na.rm = TRUE)
expect_equal(sum(actuar::dllogis(small_data, est[1], est[2], log = TRUE)),
             attr(est, "logLik"))
