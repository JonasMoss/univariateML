...
h <-\(theta) (log(16) - theta)/(theta*3)
h <-\(theta) 3/4*(3*theta - 1)
hh <- \(theta) 3/4*(theta)
...
upper <-  log(4)
a <- integrate(\(x) dexp(x, 2) * (3/2 * dexp(x, 2)/(dexp(x, 1/2) + dexp(x, 2))*x + 0.5*x), 0, upper)$value
b <- integrate(\(x) dexp(x, 1/2) * (3/2 * dexp(x, 2)/(dexp(x, 1/2) + dexp(x, 2))*x + 0.5*x), 0,  upper)$value
c <- integrate(\(x) dexp(x, 1/2) * x,  upper, Inf)$value
d <- integrate(\(x) dexp(x, 2) * x,  upper, Inf)$value
(a+b)/2 + (c+d) / 2
...
n <- 100
i <- seq(1, n)
plot(i, 2*(sqrt(i) - sqrt(i-1))/sqrt(n) / sapply(i, \(i) 1/(i * n)^(1/2)), type = "l")
lines(i, sapply(i, \(i) 1/(i * n)^(1/2)), col = "red")
