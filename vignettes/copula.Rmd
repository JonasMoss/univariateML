...
is know, the entire multivariate distribution is known too.
...
The `univariateML` can be used to speed up task 1, while the `copula` package can be used to do task 2.
...
When it fails it is since the support of the data doesn't not match the density, e.g.
when `-1` is fed to the exponential density.
